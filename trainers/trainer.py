...
def train_parser():
    ...
    parser.add_argument("--save_folder", help="Directory name to save logs and models. If not provided, saves in the current directory.", type=str, default=None)
    parser.add_argument("--cosine_anneal", help="Whether use cosine annealing for learning rate.", action="store_true")
...
class Train_Manager:
    ...
    def __init__(self,args,path_manager,train_func):
        ...
        save_dir = args.save_folder if args.save_folder else '.'
        if save_dir != '.':
            os.makedirs(save_dir, exist_ok=True)

        self.logger = get_logger(os.path.join(save_dir, '%s.log' % (name)))
        self.save_path = os.path.join(save_dir, 'model_%s.pth' % (name))
        self.writer = SummaryWriter(os.path.join(save_dir, 'log_%s' % (name)))
